...
#written for 6.190 for fall 2026 by jodalyst @ mit. based on previous work
# original original work based on Jay Lang Thesis see README.!!!
...
HTTPS_VERIFY = True #disable this for dev...
# should be out of daate now. yuck...doing this as MIT firewall patch for now. yuck.

def create_config():
    """Set up initial configuration with kerberos, MITID, and server endpoint."""
...
def compile(target_folder):
    """Submits src folder for compiling and retrieving build stuf"""
    ...
    if not os.path.isdir(src_path):
        ...
        return
    main_c_path = os.path.join(src_path, "main.c")
    if not os.path.isfile(main_c_path):
        print(f"\033[1m\033[31mError: 'main.c' not found in '{src_path}'!\033[0m")
        return
...
#but I'm worried it'll break something and tired.
def monitor(port=None, port_filter="vid=0x303A", baud=115200, wait=True, log_file=None):
    ...
    Ctrl-C to exit.
    This is a very minimal serial monitor.
    """
    if port is None:
